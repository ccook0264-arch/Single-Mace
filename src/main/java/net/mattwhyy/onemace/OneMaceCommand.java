        ...
        for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
            ...
            for (int i = 0; i < p.getInventory().size(); i++) {
                ...
                if (OneMace.isMace(s)) {
                    ...
                    src.sendFeedback(() -> Text.literal("§aThe Mace is in §b" + who + "§a's inventory."), false);
                    return 1;
                }
            }
            // ender chest
            for (int i = 0; i < p.getEnderChestInventory().size(); i++) {
                ...
                if (OneMace.isMace(s)) {
                    String who = p.getName().getString();
                    src.sendFeedback(() -> Text.literal("§aThe Mace is in §b" + who + "§a's Ender Chest."), false);
                    return 1;
                }
            }
        }
        ...
        if (!OneMace.offlineInventory.isEmpty()) {
            for (UUID uuid : OneMace.offlineInventory.keySet()) {
                if (Boolean.TRUE.equals(OneMace.offlineInventory.get(uuid))) {
                    String name = server.getUserCache().getByUuid(uuid).map(u -> u.getName()).orElse(uuid.toString());
                    final String who = name;
                    src.sendFeedback(() -> Text.literal("§eThe Mace is in offline player §b" + who + "§e's inventory."), false);
                    return 1;
                }
            }
        }
